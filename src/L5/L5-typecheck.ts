...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<{params: TExp[], exp : TExp, variable : VarRef}> => 
  !isAppExp(e) ? makeFailure("Not AppExp") :
        bind(typeofExp(e.rator, tenv), (x: TExp) =>
              isPredicateTExp(x) ? makeOk({ params: x.paramTEs, exp: x.exp, variable: e.rands[0] as VarRef }) : 
            makeFailure("Not from type of predicate"));

const typeOfIfPred = (ifExp: IfExp, tenv: TEnv): Result<TExp> => //added
    bind(isTypePredApp(ifExp.test, tenv), ({ params, exp, variable }) => {//check if is predicate
        const type = applyTEnv(tenv, variable.var)
        if(isOk(type)) {//extends the env with the new var type if exists
            if (isSubType(exp, type.value)) {
                tenv = makeExtendTEnv([variable.var], [exp], tenv);
            }
        }    
    
        const thenTE = typeofExp(ifExp.then, tenv);   //same as in normalIf     
        const altTE = typeofExp(ifExp.alt, tenv);
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>
    either(
        typeOfIfPred(ifExp, tenv),//handles case of predicate
        makeOk,
...
export const typeofProc = (proc: ProcExp, tenv: TEnv): Result<TExp> => {
    ...
    if(isPredicateTExp(proc.returnTE)) return makeOk(makeProcTExp(argsTEs, makeBoolTExp()))

     const constraint1 = bind(typeofExps(proc.body, extTEnv), (body: TExp) => 
                            checkCompatibleType(body, proc.returnTE, proc));
    return bind(constraint1, t => makeOk(makeProcTExp(argsTEs, proc.returnTE)));
};
