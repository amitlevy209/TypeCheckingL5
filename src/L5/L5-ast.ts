...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, makePredicateTExp, makeAnyTExp } from './TExp';
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        ...
        //if contains :is? creates predicate expression with an argument type any,else handle normally
        const returnTE = rest[0] === ":" ? rest[1] === "is?" ?bind( parseTExp(rest[2]),(p:TExp)=>makeOk(makePredicateTExp([makeAnyTExp()],p))): parseTExp(rest[1]) : makeOk(makeFreshTVar());
